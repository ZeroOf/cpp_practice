...
TestClient::TestClient(boost::asio::thread_pool &threadPool, const std::string &host, const std::string &service)
    : thread_pool_(threadPool), strand_(boost::asio::make_strand(threadPool)), timer_(strand_), host_(host),
      service_(service) {}
...
void TestClient::DelayConnect() {
    ...
    timer_.async_wait(
        std::bind(&TestClient::Start, std::dynamic_pointer_cast<TestClient>(shared_from_this()), pssl_context_));
}
